...
5) The server side of AWS should now be configured correctly. On the client side (which consists of your Rapsberry Pi), follow [these instructions](https://docs.aws.amazon.com/iot/latest/developerguide/connecting-to-existing-device.html) to download the necessary packages that are required to connect the Pi to the AWS account you created in step 1
